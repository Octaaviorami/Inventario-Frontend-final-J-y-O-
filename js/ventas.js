...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (saleForm) {
        saleForm.addEventListener('submit', function(e) {
            ...
            const apellido = document.querySelector('input[placeholder="Ingrese el apellido del cliente"]').value;
            const telefono = document.querySelector('input[placeholder="Ingrese el teléfono del cliente"]').value;
            const dni = document.querySelector('input[placeholder="Ingrese el DNI del cliente"]').value;
            const cantidad = parseInt(cantidadInput?.value) || 1;
            
            if (!fecha || !cliente || !apellido || !telefono || !dni) {
                alert('Por favor, completa todos los campos obligatorios');
                return;
            }
            
            // Aquí se procesaría la venta con el backend o localStorage
